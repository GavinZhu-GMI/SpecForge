...
class DFlashDataCollatorWithPadding:
    """Collator for OFFLINE DFlash training.

    Pads input_ids/attention_mask/loss_mask (2D) and the precomputed concatenated
    target hidden states (3D). Unlike DataCollatorWithPadding (EAGLE3), there is NO
    ``target`` field — DFlash computes its own block-diffusion loss from the target
    lm_head inside OnlineDFlashModel, so only ``hidden_states`` is carried.
    """
...
def prepare_dp_dataloaders(
    ...
    datacollator_cls: Optional[type] = None,
    **dataloader_kwargs,
) -> DataLoader:
    """
